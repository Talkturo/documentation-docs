...
- `GET /api/credits/auto-reload`
- `POST` `/api/credits/auto-reload` supports both session and API key authentication
- `POST` `/api/credits/claim-grants`
- `POST` `/api/credits/claim-grants/new-workspace`
- `POST` `/api/credits/welcome-bonus`
...
| `POST` | `/api/credits/auto-reload` | API key or session | Update auto-reload settings |
| `POST` | `/api/credits/claim-grants` | Session | Claim pending credit grants |
| `POST` | `/api/credits/claim-grants/new-workspace` | Session | Claim new-workspace credit grant |
| `POST` | `/api/credits/welcome-bonus` | Session | Claim welcome bonus credits |
...
### Claim new-workspace grant

`POST /api/credits/claim-grants/new-workspace` claims the one-time 20-credit ($10) grant awarded when a registered user creates their first workspace. Additional workspaces do not receive this grant.

### Claim welcome bonus

`POST /api/credits/welcome-bonus` claims a welcome bonus credit grant for the signed-in account. This endpoint is typically called once during initial onboarding.
