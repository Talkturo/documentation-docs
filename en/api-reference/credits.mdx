...
| `POST` | `/api/credits/claim-grants/new-workspace` | Session | Claim workspace signup bonus credits |
| `GET` | `/api/usage/current` | API key or session | Get current period usage summary |
| `GET` | `/api/usage/voice` | API key or session | Get voice usage breakdown |
...
### Claim new workspace bonus

`POST /api/credits/claim-grants/new-workspace` claims workspace signup bonus credits for the signed-in account. Use this endpoint after a new workspace is created to claim any promotional signup credits.

<Request tabs="cURL,JavaScript" default-tab="1">
```bash
curl -X POST "https://api.talkturo.com/api/credits/claim-grants/new-workspace" \
  -H "Cookie: talkturo_session=session_example_a8f4m2k1p9q7r3"
```
...
Returns `true` when the signup bonus is claimed successfully.

</ResponseField>

<ResponseField name="granted" field-type="number" required="true">

Number of promotional credits granted to the signed-in account.
...
Use these endpoints to inspect current-period consumption alongside the credits balance. Both endpoints support the same dual authentication model as the balance and deduction endpoints.

### Get current period usage summary

`GET /api/usage/current` returns a summary of usage for the current billing period.
...
### Get voice usage breakdown

`GET /api/usage/voice` returns a voice-specific usage breakdown including minutes, calls, and assistant activity.
...
```json
{
  ...
  "assistants": [
    ...
    {
      "assistantId": "84e72f0b-5f40-4ec2-8f29-ecc66d18d6f4",
      "name": "Support Triage",
      "minutes": 96.2,
      "calls": 41
    }
