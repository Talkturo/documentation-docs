...
`GET /api/crm/companies` supports dual authentication. Send either an API key in the `Authorization` header or an authenticated Talkturo session cookie.

All other CRM Companies endpoints on this page require session cookie authentication and must be sent from an authenticated Talkturo session.
...
| --- | --- | --- | --- |
| `GET` | `/api/crm/companies` | Dual auth | List companies |
| `POST` | `/api/crm/companies` | Session | Create a company |
| `GET` | `/api/crm/companies/{id}` | Session | Retrieve a company |
| `PUT` | `/api/crm/companies/{id}` | Session | Update a company |
| `DELETE` | `/api/crm/companies/{id}` | Session | Delete a company |
| `GET` | `/api/crm/companies/{id}/contacts` | Session | List contacts for a company |
...
`POST /api/crm/companies` is deprecated and returns HTTP `410`. Use `POST /api/crm/groups` to create groups instead.
...
Deleting a company is blocked when contacts are still attached to that company. Remove the associated contacts first, then delete the company.
