...
```bash
curl -X POST "https://app.talkturo.com/api/auth/sign-in" \
...
```javascript
const response = await fetch("https://app.talkturo.com/api/auth/sign-in", {
  method: "POST",
  headers: {
...
<Request tabs="cURL,JavaScript" default-tab="1">
```bash
curl -X POST "https://app.talkturo.com/api/auth/refresh" \
  -H "Content-Type: application/json" \
  -d '{
    "refresh_token": "refresh_example_q9K2mL7xP4vN8cR1"
  }'
...
```javascript
const response = await fetch("https://app.talkturo.com/api/auth/refresh", {
  method: "POST",
  headers: {
    "Content-Type": "application/json"
  },
