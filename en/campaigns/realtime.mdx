...
If you need to verify how the campaign was set up before it started, compare what you see here with the campaign settings in [Campaign configuration](/en/campaigns/configuration).
...
Use the **View** link when a single call needs deeper analysis, such as transcripts, messages, cost, or recordings. The dashboard links into [Call Logs](/en/call-logs) with the campaign filter already applied.
...
For historical analysis across all campaign calls, move to [Call Logs](/en/call-logs). For campaign setup details, return to [Campaign configuration](/en/campaigns/configuration).

## Related pages

<Columns cols={3}>
  <Card title="Campaign overview" href="/en/campaigns/overview" icon="bar-chart" cta="Open guide" />
  <Card title="Campaign configuration" href="/en/campaigns/configuration" icon="settings" cta="Review settings" />
  <Card title="Call Logs" href="/en/call-logs" icon="phone" cta="Inspect calls" />
</Columns>